 ...
 * 类并不属于多线程间的通信，⽽是让每个线程有⾃⼰”独⽴“的变量，线程之间互不 影响。它为每个线程都创建⼀个副本，每个线程可以访问⾃⼰内部的副本变量。
 *
 * InheritableThreadLocal类与ThreadLocal类稍有不同，Inheritable是继承的意思。 它不仅仅是当前线程可以存取副本值，⽽且它的⼦线程也可以存取这个副本值。
 *
 * @author karl xie
 * Created on 2020-04-15 21:00
 */
public class ThreadLocalDemo {
     ...
     * 但是它们各⾃可以存取⾃⼰当前线程的⼀个值。 那ThreadLocal有什么作⽤呢？如果只是单纯的想要线程隔离，在每个线程中声明
    ...
    */
    public static void main(String[] args) throws InterruptedException {
        ...
        Thread a = new Thread(new ThreadA(threadLocal));
        Thread b = new Thread(new ThreadB(threadLocal));
        a.start();
        //a.join();
        b.start();
    }
